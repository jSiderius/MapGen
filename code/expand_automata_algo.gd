...
func expand_id_array(idArray : Array, autonomous_ids : Array = [], empty_space_expands : bool = false) -> Array:
...
func expand_id_array_instance(idArray : Array, autonomous_ids : Array = [], empty_space_expands : bool = false) -> Array: 
	...
	for x in range(len(idArray)): 
		for y in range(len(idArray[x])): 
			...
			if (idArray[x][y] <= 2 and not empty_space_expands) or idArray[x][y] <= 1 : continue 
			...
			for n in candidateNeighbors: 
				if is_valid_expansion_candidate(idArrayNew, Vector2(x+n[0], y+n[1]), idArrayNew[x][y], 1): 
...
func get_candidate_expansion_neighbors(idArrayArg : Array, pos : Vector2, autonomous_ids : Array = []) -> Array: 
...
func is_valid_expansion_candidate(idArray : Array, pos : Vector2, id : int, ttl : int) -> bool: 
	...
	if (idArray[x][y] > 2 and not idArray[x][y] == id) or idArray[x][y] == -3: return false
	
	# Check all neighbors recursively with updated ttl 
	for n in neighbors: 
		if not is_valid_expansion_candidate(idArray, Vector2(x + n[0], y + n[1]), id, ttl-1): return false
